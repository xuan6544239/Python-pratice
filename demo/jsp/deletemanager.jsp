        ...
        <table>
            ...
            ResultSet rs2;
						...
						{%>
							...
							<td><%out.print(rs2.getString("Product_Id"));%></td>
							<td><%out.print(rs2.getString("Product_Code"));%></td>
							<td><%out.print(rs2.getString("Product_brand"));%> </td>
							<td><%out.print(rs2.getString("Product_Name"));%> </td>
							<td><%out.print(rs2.getString("Product_FM"));%></td>
							<td><%out.print("$"+rs2.getString("Product_Price"));%> </td>
							<td><%out.print(rs2.getString("Product_Inventory"));%> </td>
              <td><img src="../assets/images/<%out.print(rs2.getString("Product_Code")+".jpg");%>" width="250px"></p></td>
							<td><%out.print(rs2.getString("Product_Note"));%> </td>

						<form action="delete.jsp" method="POST">
							<td><input type="submit"  value="刪除" ></td>
							<input type="hidden" name="Product_Id" value="<%=rs2.getString("Product_Id")%>">
							<input type="hidden" name="Product_Code" value="<%=rs2.getString("Product_Code")%>">
							<input type="hidden" name="Product_brand" value="<%=rs2.getString("Product_brand")%>">
							<input type="hidden" name="Product_Name" value="<%=rs2.getString("Product_Name")%>">	           
							<input type="hidden" name="Product_FM" value="<%=rs2.getString("Product_FM")%>">
							<input type="hidden" name="Product_Price" value="<%=rs2.getString("Product_Price")%>">
							<input type="hidden" name="Product_Inventory" value="<%=rs2.getString("Product_Inventory")%>">
							<input type="hidden" name="Product_Note" value="<%=rs2.getString("Product_Note")%>">	
						</form>	
    	    		</tr>
    	    	<%}%>	
    	    	</table>
